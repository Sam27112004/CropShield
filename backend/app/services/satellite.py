...
class SatelliteService:
    ...
    def _fetch_earth_engine_scene(
        ...
        *,
        ...
        source_label = "Google Earth Engine (cloud-masked)"
        if valid_ratio < 0.03:
            image = ee.Image(collection.median()).setDefaultProjection(native_projection).clip(region)
            sampled = image.select(["B2", "B3", "B4", "B8"]).sampleRectangle(region=region, defaultValue=0).getInfo()
            properties = sampled.get("properties", {})
            bands = {
                band: np.asarray(properties[band], dtype=np.float32)
            ...
            source_label = "Google Earth Engine (cloud-limited composite)"
